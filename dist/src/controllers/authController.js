...
const sendVerificationEmail = (user, verificationUrl) => __awaiter(void 0, void 0, void 0, function * () {
  try {
    yield new Email_1.Email(user.email, 'Verify your email', 'Please verify your email by clicking on the button above', (0, emailTemplates_1.getVerificationEmailTemplate)(user, verificationUrl)).send()
  } catch (error) {
...
exports.signup = (0, asyncHandler_1.asyncHandler)((req, res, next) => __awaiter(void 0, void 0, void 0, function * () {
  ...
  if (existingUser) { return next(new AppError_1.AppError('Email already taken', 400)) }
  ...
  yield sendVerificationEmail(createdUser, verificationUrl)
  res.status(201).json({
    status: 'success',
    success: true,
    user
...
exports.login = (0, asyncHandler_1.asyncHandler)((req, res, next) => __awaiter(void 0, void 0, void 0, function * () {
  ...
  if (!user) { return next(new AppError_1.AppError('User with this email does not exist!', 404)) }
  if (!user.emailVerified) { return next(new AppError_1.AppError('Your email has not been verified yet!', 400)) }
  const isCorrectPassword = user.isValidPassword(password)
  if (!isCorrectPassword) { return next(new AppError_1.AppError('Wrong password!', 404)) }
  const payload = {
    _id: user._id,
    email: user.email,
    role: user.role,
...
exports.verifyEmail = (0, asyncHandler_1.asyncHandler)((req, res, next) => __awaiter(void 0, void 0, void 0, function * () {
  ...
  const user = yield userModel_1.User.findOneAndUpdate({ emailVerificationCode }, { emailVerified: true }, { new: true })
  if (!user) { return next(new AppError_1.AppError('Code validation fail', 500)) }
...
exports.forgotPassword = (0, asyncHandler_1.asyncHandler)((req, res, next) => __awaiter(void 0, void 0, void 0, function * () {
  ...
  if (!email) { return next(new AppError_1.AppError('Email is required!', 400)) }
  if (!isValidEmail(email)) { return next(new AppError_1.AppError('Email is not valid!', 400)) }
  const user = yield userModel_1.User.findOne({ email })
  if (!user) { return next(new AppError_1.AppError('User not exist!', 404)) }
...
exports.resetPassword = (0, asyncHandler_1.asyncHandler)((req, res, next) => __awaiter(void 0, void 0, void 0, function * () {
  const { password } = req.body
  if (!password || password.length < 6) { return next(new AppError_1.AppError('Password must be greater than 6 characters!', 400)) }
  ...
  if (!user) { return next(new AppError_1.AppError('Token is invalid or has expired', 400)) }
